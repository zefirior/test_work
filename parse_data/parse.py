...
def produce_page_task(manager, code):
    def task():
        for page in get.produce_insider_page(code)[:10]:
            manager.queue.append(insider_task(code, page))
        print(f"For ticker {code} will be parse insider info in pages:1-{page}")
    return task


def insider_task(code, page):
    def task():
        ...
        while True:
            try:
                ...
                db.session.commit()
                break
            except (InvalidRequestError, IntegrityError, OperationalError):
                db.session.rollback()
                retry -= 1
                if not retry:
                    raise
...
def historical_task(code):
    def task():
        ...
        ticker = q.ticker_set_default(code=code)
        for row in get.historical(code):
